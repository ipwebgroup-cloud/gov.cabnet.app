<?php
/**
 * gov.cabnet.app — Bolt pre-ride email parser v3 isolated.
 *
 * Safety:
 * - No database access.
 * - No network calls.
...
final class BoltPreRideEmailParserV3
{
    ...
    public function parse(string $raw): array
    {
        ...
        $fields = [
            'operator' => $this->pick($labelMap, ['operator', 'fleet', 'provider']),
            'customer_name' => $this->pick($labelMap, ['customer', 'client', 'passenger', 'customer name']),
        ...
        if ($fields['start_time_text'] !== '' && $fields['estimated_pickup_time_text'] === '') {
            $warnings[] = 'Estimated pick-up time was not found; Start time is used as pickup datetime fallback.';
        }
        ...
        return [
            ...
            'source' => 'bolt_pre_ride_email_v3_isolated',
            'safety' => [
    ...
    /** @param array<string,string> $map @param array<int,string> $keys */
    ...
    private function extractPriceAmount(string $text): string
    {
        ...
        preg_match_all('/([0-9]+(?:[,.][0-9]{1,2})?)/', $text, $matches);
        $numbers = $matches[1] ?? [];
        if (empty($numbers)) {
            return '';
        }
        // v3 business rule: when Bolt gives a range, use the upper/final value to avoid understatement.
        $amount = str_replace(',', '.', (string)end($numbers));
    ...
    /** @param array<int,string> $missing @param array<int,string> $warnings */

 ...
 *
 * v3.1.2:
 * - Read-only classifier for V3 queue rows blocked by expiry/future-safety guards.
 * - Helps distinguish historical real-mail rows from canary rows after the closed-gate observation phase.
 * - No Bolt calls, no EDXEIX calls, no AADE calls, no DB writes, no queue mutations.
 */

declare(strict_types=1);

const GOV_V3_REAL_MAIL_EXPIRY_AUDIT_VERSION = 'v3.1.2-v3-real-mail-expiry-reason-audit';
...
function gov_v3_expiry_db(): array
{
    ...
    try {
        ...
        return [
            ...
            'error' => '',
            'connection' => $mysqli,
            'name' => method_exists($mysqli, 'query') ? (string)($mysqli->query('SELECT DATABASE() AS db')->fetch_assoc()['db'] ?? '') : '',
...
function gov_v3_expiry_fetch_rows(mysqli $mysqli, string $table, array $columns, int $limit = 50): array
{
    $wanted = [
        'id',
        'dedupe_key',
    ...
    foreach ($wanted as $column) {
        if (!empty($columns[$column])) {
            $select[] = gov_v3_expiry_identifier($column);
        }
    }
...
function gov_v3_expiry_classify_row(array $row): array
{
    ...
    $isCanary = (
        stripos($customer, 'canary') !== false
        || stripos($orderRef, 'CANARY') !== false
        || stripos($operatorNote, 'canary') !== false
    );

    $isPossibleRealMail = !$isCanary && (
        $customer !== ''
        || $orderRef !== ''
        || $sourceMailbox !== ''
    );
...
function gov_v3_real_mail_expiry_reason_audit_run(): array
{
    ...
    $report = [
        ...
        'mode' => 'read_only_v3_real_mail_expiry_reason_audit',
        'started_at' => date('c'),
        ...
        'warnings' => [],
        'recommended_next_step' => 'Continue observing real-mail intake. Do not submit, move, delete, or redirect anything.',
        'final_blocks' => [],
        'finished_at' => null,
    ];
    ...
    $rows = gov_v3_expiry_fetch_rows($mysqli, $table, $columns, 50);
    ...
    foreach ($rows as $row) {
        ...
        if (($item['queue_status'] ?? '') === 'blocked') {
            $report['summary']['blocked_rows_reviewed']++;
        }
        if (!empty($item['pickup_future_now'])) {
            $report['summary']['future_pickup_rows_in_error_set']++;
        }
        if (!empty($item['submitted_at'])) {
            $report['summary']['submitted_rows_in_error_set']++;
        }
        if (!empty($item['failed_at'])) {
            $report['summary']['failed_rows_in_error_set']++;
        }

        $classification = (string)($item['classification'] ?? 'other');
        if (array_key_exists($classification, $report['summary'])) {
...
function gov_v3_real_mail_expiry_reason_audit_main(array $argv): void
{
    ...
    echo 'Expired by guard: ' . (string)($report['summary']['expired_by_future_safety_guard'] ?? 0) . PHP_EOL;

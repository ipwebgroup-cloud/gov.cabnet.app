...
const V3_SUBMIT_PREFLIGHT_VERSION = 'v3.0.12-submit-preflight-dry-run';

function v3sp_usage(): string
{
    return "Usage:\n"
        . "  php pre_ride_email_v3_submit_preflight.php [--limit=20] [--status=queued|ready|all] [--json]\n\n"
        . "Safety:\n"
...
/** @return array{ready:bool,blocks:array<int,string>,warnings:array<int,string>,minutes_until:int|null} */
function v3sp_preflight_row(array $row): array
{
    $blocks = [];
    $warnings = [];
    $minutesUntil = null;
...
if (!in_array($status, ['queued', 'ready', 'all'], true)) {
    $status = 'queued';
}
...
try {
    ...
    $queueOk = v3sp_table_exists($db, 'pre_ride_email_v3_queue');
    $eventsOk = v3sp_table_exists($db, 'pre_ride_email_v3_queue_events');
    ...
    foreach ($rows as $index => $row) {
        $preflight = v3sp_preflight_row($row);
        if ($preflight['ready']) {
...
echo "Schema OK: " . (!empty($result['schema_ok']) ? 'yes' : 'no') . PHP_EOL;
if ($result['error'] !== '') {

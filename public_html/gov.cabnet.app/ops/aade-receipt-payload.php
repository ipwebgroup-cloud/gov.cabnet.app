...
$out = [
    ...
    'generated_at' => date('c'),
    'verdict' => $error ? 'AADE_PAYLOAD_PREVIEW_ERROR' : ($preview ? 'AADE_PAYLOAD_PREVIEW_READY' : 'AADE_PAYLOAD_SELECT_BOOKING'),
    'safety_contract' => [
        'read_only' => true,
...
<div class="container py-4">
  ...
  <div class="row g-3 mb-3">
    <div class="col-md-5"><div class="card"><div class="card-body"><h2 class="h5">Config</h2><pre class="mono small mb-0"><?= h(json_encode($out['config'], JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)) ?></pre></div></div></div>
    <div class="col-md-7"><div class="card"><div class="card-body"><h2 class="h5">Preview</h2><?php if ($error): ?><div class="alert alert-danger"><?= h($error) ?></div><?php elseif ($preview): ?><pre class="mono small"><?= h(json_encode($preview, JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)) ?></pre><?php else: ?><div class="text-muted">Select a booking using <code>?booking_id=ID</code>.</div><?php endif; ?></div></div></div>
  </div>

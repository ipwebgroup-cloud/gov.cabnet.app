...
<head>
    ...
    <style>
        ...
        * { box-sizing:border-box; }
        ...
        .wrap { width:min(1480px, calc(100% - 48px)); margin:26px auto 60px; }
...
<main class="wrap">
    ...
    <section class="card">
        ...
        <p class="small">Unmapped drivers need a real EDXEIX driver ID before a matching future Bolt trip can be live-ready.</p>
        <?php if (!$state['drivers']): ?>
            <p>No driver rows match the current filter.</p>
        <?php else: ?>
            <div class="table-wrap"><table>
                <thead><tr><th>Status</th><th>ID</th><th>Source</th><th>Bolt Driver UUID</th><th>Driver Name</th><th>Phone</th><th>EDXEIX Driver ID</th><th>Active Vehicle UUID</th><th>Active Plate</th><th>Active</th><th>Last Seen</th></tr></thead>
                <tbody>
                <?php foreach ($state['drivers'] as $row): $mapped = map_is_mapped($row, ['edxeix_driver_id', 'driver_id']); ?>
    ...
    <section class="card">
        ...
        <p class="small">Unmapped vehicles need a real EDXEIX vehicle ID before a matching future Bolt trip can be live-ready.</p>
        <?php if (!$state['vehicles']): ?>
            <p>No vehicle rows match the current filter.</p>
        <?php else: ?>
            <div class="table-wrap"><table>
                <thead><tr><th>Status</th><th>ID</th><th>Source</th><th>Bolt Vehicle UUID</th><th>Plate</th><th>Name</th><th>Model</th><th>EDXEIX Vehicle ID</th><th>Active</th><th>Last Seen</th></tr></thead>
                <tbody>
                <?php foreach ($state['vehicles'] as $row): $mapped = map_is_mapped($row, ['edxeix_vehicle_id', 'vehicle_id']); ?>
                    <tr class="<?= $mapped ? '' : 'unmapped' ?>">
                        <td><?= map_yes_badge($mapped) ?></td>
                        <td><?= map_h(map_value($row, ['id'], '')) ?></td>
                        <td><?= map_h(map_value($row, ['source_system', 'source_type', 'source'], '')) ?></td>
